...
<div id="achievements">
    <div class="row ma-5 bg-white">
        ...
        <AchievementViewDialog :viewDialog="viewDialog" :viewLang="viewLang" :loading="loading" :selectedAchievement="selectedAchievement" :eventType="eventType" @edit="submit($event, 'edit')" @add="submit($event, 'add')" @closeEditDialog="closeDialog($event, 'view')" @closeAddDialog="closeDialog($event, 'view')" />
        ...
        <VCard itemKey="slugTranslation" :actionsTable="actionsTable" :data="data" :meta="meta" :loading="loading" @OpenDialog="openDialog($event)" @OpenViewEnDialog="openViewDialog($event, 'en')" @OpenViewArDialog="openViewDialog($event, 'ar')" @openDeleteDialog="openDeleteDialog($event)" @newPage="fetchData($event)"></VCard>
    </div>
</div>
</template>
...
import AchievementViewDialog from '../../components/view-achievement-dialog.vue'
